...
> **Nota de reconciliación (archive, 2026-09-05)**: este texto refleja el comportamiento REALMENTE entregado tras los refinamientos de QA del tester (Slices E y E.b), no la propuesta original. Cambios post-propuesta: (1) la ruta se renombró de `/ventas/notas-credito` a `/ventas/facturas-emitidas`; (2) los 3 campos de filtro separados (`nro_factura`/cliente/RIF en Facturas, `nro`/cliente/RIF/tipo en NC) se reemplazaron por UN buscador único por pestaña; (3) el filtro de estado de Facturas se "foldeó" dentro del buscador como palabras clave, no un `<select>` separado, y ganó el estado `ABONADA` (no existía en la propuesta); (4) la pestaña NC perdió por completo el filtro de estado/tipo y el botón "Ver todo el historial" — el rango de fecha (default mes actual) es su único control de amplitud, sin escape hatch; (5) el buscador previo de `notas-credito-page.tsx` (`useBuscarFacturaParaAnular`) se ELIMINÓ, no se preservó — la pestaña Facturas es ahora el único punto de entrada para seleccionar una factura y aplicarle NC; (6) se agregó atenuación visual de filas 100% reversadas (no estaba en la propuesta).
