...
**Deviation (RESOLVED post-Slice-4, obs #2954)**: admin "Devolver dinero" originally dispatched `modalidad: 'EFECTIVO_REAL'` as a workaround for a stale guard (`use-notas-credito.ts` ~line 506) throwing `"REFUND_TESORERIA aun no esta implementado"`. Fixed in a follow-up amendment on this same branch (`feat/ncr-cuadre-04-selector-guard`): the phantom throw was removed (the two-pass write core never keyed off `EFECTIVO_REAL` specifically — only `movesCash`/`tipoNc`, confirmed by reading the full write core), the dedicated throw-asserting test was flipped to an end-to-end success test (+1 mixed-type test), and `crear-ncr-modal.tsx` now dispatches `REFUND_TESORERIA` for money refunds (POS keeps `EFECTIVO_REAL`, matching the pre-existing intent documented in `nota-credito-pos-modal.tsx`'s own comment reserving `REFUND_TESORERIA` for the Tradicional module). Verified: 95 files / 1231 tests pass (+1 net), `yarn type-check`/`yarn type-check:test` clean.
...
## Phase 5: Cuadre (Slice 5) — BASE scope (owner-confirmed split)

**SCOPE BOUNDARY (owner decision)**: Slice 5 implements the BASE cuadre reflection of the money engine — session-scoped NC total, per-method refunds (with `nro_ncr` traceability), the session's NC table (contado/crédito split), and confirming `useSaldoEfectivoBimonetario` nets correctly. This is the direct, verifiable consequence of the money engine built in Slices 2–4.
...
## Phase 7: Hardening (fresh-context verify-report warnings W1/W2) — DONE

Fresh-context adversarial verify (`verify-report.md`, PASS WITH WARNINGS) found two non-CRITICAL gaps after all 6 slices landed. Addressed here, on the same chain tip (`feat/ncr-cuadre-06-badge`), new commit, never `--amend`.

- [x] 7.1 W1 — precision convention: swapped `toStorageString` (8dp) for `.toFixed(4)` on the treasury/bank/`metodos_cobro`-BALANCE writes design.md Decision 1 specifies as 4dp (matching `use-traspasos.ts:938`'s convention for these exact columns): `mov_caja_fuerte` monto/saldo_anterior/saldo_nuevo, `caja_fuerte.saldo_actual`, `movimientos_bancarios` monto/saldo_anterior/saldo_nuevo, `bancos_empresa.saldo_actual`, `metodos_cobro.saldo_actual`. Kept `toStorageString` on `movimientos_metodo_cobro`'s OWN ledger-row fields (per `use-ventas.ts:825`'s convention for that table — verify-report explicit instruction). Format-only change, no arithmetic/comparison touched. Updated 6 existing test assertions in `use-notas-credito.test.ts` that pinned the old 8dp format on the affected writes (metodos_cobro UPDATE, movimientos_bancarios INSERT ×3, bancos_empresa UPDATE ×2) to expect 4dp.
- [x] 7.2 W2 RED→GREEN — function-level guard: added a pre-tx throw (`use-notas-credito.ts`, right after `validarOrigenDinero`, before `writeTransaction` opens) for `tipoNc==='PARCIAL' && movesCash` (desembolso modalidad + populated `origenDinero` on a PARCIAL NC) — mirrors `assertGateAntiFraudeNoDesembolso`'s "reject at function level, not UI level" philosophy. Without this guard the write core silently rerouted the full remanente to SAFC credit, ignoring the caller's cash request. 3 new tests (RED confirmed by temporarily disabling the guard — both desembolso combos failed on an unrelated "linea no pertenece a la factura" error instead of the expected `/PARCIAL/i` message, proving the guard runs before any other pre-tx logic once re-enabled): PARCIAL+`EFECTIVO_REAL`+origenDinero → throws; PARCIAL+`REFUND_TESORERIA`+origenDinero → throws; PARCIAL+`AJUSTE_CXC` (no desembolso) → still succeeds (guard correctly scoped, does not regress Deviation 2's documented AJUSTE_CXC path).
- [x] 7.3 Verify green: `yarn test:run` 97 files / 1255 tests (was 1252, +3 net). `yarn type-check` clean on all non-test files (4830 total errors, all pre-existing vitest-globals noise on `*.test.ts(x)`, confirmed via `grep -v test.ts`). `yarn type-check:test` clean, 23.5s. NOT pushed — executor scope local-only, no push/PR/merge.
